...
X_train = Matrix(CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Code\\Final Mobility model\\X_train_final.csv", DataFrame))
X_test = Matrix(CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Code\\Final Mobility model\\X_test_final.csv", DataFrame))
y_train = String.(collect(vec(CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Code\\Final Mobility model\\y_train_final.csv", DataFrame)[:,1])))
y_test = String.(collect(vec(CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Code\\Final Mobility model\\y_test_final.csv", DataFrame)[:,1])))
train_indices = vec(CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Code\\Final Mobility model\\train_indices_final.csv", DataFrame)[:,1])
test_indices = vec(CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Code\\Final Mobility model\\test_indices_final.csv", DataFrame)[:,1])
...
test_counts = countmap(test_inchis)

# Step 5: Count how many times each InChI was misclassified
misclass_counts = countmap(misclassified_inchis)

# Step 6: Combine info: show misclassifications per compound
results = [(inchi, misclass_counts[inchi], test_counts[inchi]) for inchi in keys(misclass_counts)]

# Optional: sort by misclassification frequency
sorted_results = sort(results, by = x -> x[2], rev = true)

# Now `sorted_results` contains tuples like:
# ("InChI=1S/C...", 3, 5) → meaning: misclassified 3 times out of 5 test appearances
...
y_hat_test = string.(y_hat_test)
y_test = String.(collect(y_test))
