...
unique_train = string.(unique(merged_data.SMILES[train_indices]))
original_data = string.(merged_data.SMILES[train_indices])
y = merged_data.Class[train_indices]
...
train_fold = []
test_fold = []
unique_indices =  string.(unique(merged_data.SMILES[train_indices]))
...
test_fold_1, train_fold_1 = append_occurrences_folds(unique_train, 1, train_indices)

###Saving
df_train_fold_1 = DataFrame(indices = train_fold_1)
CSV.write("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Train test folds merged expanded data\\train_fold_1.csv", df_train_fold_1)
df_test_fold_1 = DataFrame(indices = test_fold_1)
CSV.write("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Train test folds merged expanded data\\test_fold_1.csv", df_test_fold_1)

test_fold_2,train_fold_2 = append_occurrences_folds(unique_train, 2, train_indices) 
#Saving
df_train_fold_2 = DataFrame(indices = train_fold_2)
CSV.write("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Train test folds merged expanded data\\train_fold_2.csv", df_train_fold_2)
df_test_fold_2 = DataFrame(indices = test_fold_2)
CSV.write("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Train test folds merged expanded data\\test_fold_2.csv", df_test_fold_2)

test_fold_3, train_fold_3 = append_occurrences_folds(unique_train, 3, train_indices) 
#Saving
df_train_fold_3 = DataFrame(indices = train_fold_3)
CSV.write("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Train test folds merged expanded data\\train_fold_3.csv", df_train_fold_3)
df_test_fold_3 = DataFrame(indices = test_fold_3)
CSV.write("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Train test folds merged expanded data\\test_fold_3.csv", df_test_fold_3)

train_fold_1 = CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Train test folds merged expanded data\\train_fold_1.csv", DataFrame).indices
test_fold_1 = CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Train test folds merged expanded data\\test_fold_1.csv", DataFrame).indices
train_fold_2 = CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Train test folds merged expanded data\\train_fold_2.csv", DataFrame).indices
test_fold_2 = CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Train test folds merged expanded data\\test_fold_2.csv", DataFrame).indices
train_fold_3 = CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Train test folds merged expanded data\\train_fold_3.csv", DataFrame).indices
test_fold_3 = CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Train test folds merged expanded data\\test_fold_3.csv", DataFrame).indices
...
df_results = CSV.read("R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Merged and expanded CV 3 optimisation results.csv", DataFrame)

#Setting up cross-validation
function CV_3_RFC(n_estimatorss, max_featuress, max_depths, min_samples_splits, min_samples_leafs, criterions)
    ...
    rf_cl = RandomForestClassifier(n_estimators = n_estimatorss, max_features = max_featuress,
                                   ...
                                   min_samples_leaf = min_samples_leafs, criterion = criterions,
...
function random_search_rfc(n_iter::Int)
    # Define hyperparameter search space
    n_estimatorss = [50, 100]
    max_featuress = [0.2, 0.5, 0.7]
    max_depths = [25, 50 , 100]
    min_samples_splits = [2, 4, 6, 10, 15, 30]
    min_samples_leafs = [2, 4, 6, 10, 15, 30]
    criterions = ["gini", "log_loss", "entropy"]

    

    # Define safe file path and ensure directory exists
    save_path = raw"R:\\PHD2024TH-Q6813\\Research files\\Models and other documents\\Merged and expanded CV 3 optimisation results.csv"
    ...
    for i in 1:n_iter
        ...
        n_estimators      = rand(n_estimatorss)
        max_features      = rand(max_featuress)
        max_depth         = rand(max_depths)
        min_samples_split = rand(min_samples_splits)
        min_samples_leaf  = rand(min_samples_leafs)
        criterion         = rand(criterions)

        @show i, n_estimators, max_features, max_depth, min_samples_split, min_samples_leaf, criterion

        # Run cross-validation
        train_score, test_score, F1_train, F1_test = CV_3_RFC(n_estimators, max_features, max_depth, min_samples_split, min_samples_leaf, criterion)

        # Save results in memory
        push!(df_results, [
            n_estimators, max_features, max_depth, min_samples_split, min_samples_leaf, criterion,
            train_score, test_score, F1_train, F1_test
        ])
        println(F1_test)
        # Save to disk every iteration (or customize n_save)
        println("Saving after iteration $i")
        CSV.write(save_path, df_results)
    end
...
# Initialize results DataFrame
    df_results = DataFrame(
        ...
        CV_3_test_score = Float64[],
        CV_3_train_F1 = Float64[],
        CV_3_test_F1 = Float64[]
    )
# Example usage:
#df_random_results = random_search_rfc(1000)
